...
// against a real REW measurement pair (baseline vs filter-applied).
//
// This exists to replace ad hoc single-point delta checks ("read magnitude
// at 1kHz") with a real fit across the whole frequency range the filter
// should affect: RMSE, max error, and correlation between the measured delta
// curve and the theoretically-expected one.
//
...
function peakingMagnitudeDb(centerFreq, gainDb, q, evalFreq, fs = 96000) {
  return biquadMagnitudeDb(biquadCoefficients('PK', centerFreq, gainDb, q, fs), evalFreq, fs);
}

function theoreticalMagnitudeDb(filterSpec, evalFreq, fs = 96000) {
...
function logSpacedFrequencies(minFreq, maxFreq, count) {
...
//
// `inBandOnly`: if true, restrict stats to points where the theoretical
// effect exceeds `inBandThresholdDb` (default 0.5dB) — i.e. where the filter
// should actually be doing something, so out-of-band noise doesn't dilute
// the fit. Also returns outOfBand stats (should be ~0dB delta, flat) as a
// separate check that the filter isn't leaking effect where it shouldn't.
...
function compareToTheoretical(baselineFr, testFr, filterSpec, {
  magnitudeAt,
...
function judgeFit(comparison, { rmseToleranceDb = 1.5, correlationMin = 0.8, outOfBandRmseToleranceDb = 1.0 } = {}) {
...
function estimateEffectiveQ(baselineFr, measuredFr, filterSpec, {
  magnitudeAt,
