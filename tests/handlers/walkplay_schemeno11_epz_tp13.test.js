/**
 * EPZ TP13 AI ENC audio (WalkPlay SchemeNo11) — auto-generated tests
 * Capture: tests/captures/walkplay_schemeno11_epz_tp13.json
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/walkplay_schemeno11_epz_tp13.json');
  await mock.open();
...
export async function test_pullFromDevice_returns8Bands(assert) {
  ...
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  const defined = result.filters.filter(f => f !== undefined);
  assert.equal(defined.length, 8, 'SchemeNo11 EPZ TP13 should return 8 bands');
}
