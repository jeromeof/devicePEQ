/**
 * NiceHCK Octave (WalkPlay SchemeNo18) — auto-generated tests
 * Capture: tests/captures/walkplay_schemeno18_nicehck_octave.json
 * vendorId=0x3302  productId=0x39C3
 *
 * walkplayPeq10Band10dBFullShelves — 10 bands, ±10 dB, LS+HS, pregain
 * productId added to SchemeNo18 productIds list (was missing, device fell back to 8-band default)
 */
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/walkplay_schemeno18_nicehck_octave.json');
  await mock.open();
...
export async function test_pullFromDevice_returns10Bands(assert) {
  ...
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  const defined = result.filters.filter(f => f !== undefined);
  assert.equal(defined.length, 10, 'SchemeNo18 Octave should return 10 bands (not 8 — was a config bug)');
}
...
export async function test_pullFromDevice_gainsWithin6dBCeiling(assert) {
  ...
  result.filters.filter(f => f !== undefined).forEach((f, i) => {
    assert.ok(f.gain >= -12 && f.gain <= 6,
      `filter[${i}].gain ${f.gain} should be within SchemeNo18 range [-12, +6]`);
  });
}
