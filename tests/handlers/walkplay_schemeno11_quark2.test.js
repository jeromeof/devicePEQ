/**
 * Quark2 (WalkPlay SchemeNo11) — auto-generated tests
 * Capture: tests/captures/walkplay_schemeno11_quark2.json
 * vendorId=0x3302  productId=0x12DB
 *
 * walkplayPeq8Band10dBLsLowpass — 8 bands, ±10 dB, LS + LP/HP filters (no HS), pregain
 */
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/walkplay_schemeno11_quark2.json');
  await mock.open();
...
export async function test_pullFromDevice_returns8Bands(assert) {
  ...
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  const defined = result.filters.filter(f => f !== undefined);
  assert.equal(defined.length, 8, 'SchemeNo11 should return 8 filter bands');
}
...
export async function test_pushToDevice_writesGlobalGain(assert) {
  ...
  assert.ok(gainWrite !== undefined, 'SchemeNo11 (deviceHandlesPregain=false) should write global gain');
}
