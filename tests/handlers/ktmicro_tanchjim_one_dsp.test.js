/**
 * TANCHJIM-ONE DSP (KT Micro) — auto-generated tests
 * Capture: tests/captures/ktmicro_tanchjim_one_dsp.json
 * vendorId=0x31B2  productId=0x0111
 *
 * peq5Band12dBFullShelvesNoPregain — 5 bands, ±12 dB, LS+HS, no pregain
 * Reads 10 registers at baseRegisterOffset=0x26 (2 regs per band)
 */
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/ktmicro_tanchjim_one_dsp.json');
  await mock.open();
...
// ── pushToDevice ───────────────────────────────────────────────────────────────
// KT Micro ONE DSP: fire-and-forget writes (no ACK from device).
// Uses plain device.sendReport — NOT sendCommandWithResponse for writes.
...
export async function test_pushToDevice_sendsCommit(assert) {
  ...
  // Commit packet: [0x00, 0, 0, 0, COMMAND_COMMIT=0x53, ...]
  const commitSend = mock.sentBytes.find(b => b.length > 4 && b[4] === 0x53);
  assert.ok(commitSend !== undefined, 'should send commit command (0x53)');
}

export async function test_pushToDevice_noAckRequiredForWrites(assert) {
  // Verify that pushToDevice completes even though writes return no responses.
  // This confirms the fire-and-forget behaviour for devices that don't ACK writes.
  const mock = await loadCapture('../captures/ktmicro_tanchjim_one_dsp.json');
  await mock.open();
  const details = makeDeviceDetails(mock);
  ...
  assert.ok(!threw, 'pushToDevice should not throw even without write ACKs');
}
