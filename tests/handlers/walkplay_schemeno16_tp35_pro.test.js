/**
 * TP35 Pro (WalkPlay SchemeNo16) — auto-generated tests
 * Capture: tests/captures/walkplay_schemeno16_tp35_pro.json
 * vendorId=0x3302  productId=0x43E6
 *
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/walkplay_schemeno16_tp35_pro.json');
  await mock.open();
...
export async function test_pullFromDevice_returns10Bands(assert) {
  ...
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  const defined = result.filters.filter(f => f !== undefined);
  assert.equal(defined.length, 10, 'SchemeNo16 TP35 Pro should return 10 bands');
}
