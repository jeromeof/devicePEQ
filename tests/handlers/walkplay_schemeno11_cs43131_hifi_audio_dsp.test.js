/**
 * CS43131 HiFi Audio DSP (WalkPlay SchemeNo11) — auto-generated tests
 * Capture: tests/captures/walkplay_schemeno11_cs43131_hifi_audio_dsp.json
 * vendorId=0x3302  productId=0x51C0
 *
...
export async function test_pullFromDevice_returnsFilters(assert) {
  const mock = await loadCapture('../captures/walkplay_schemeno11_cs43131_hifi_audio_dsp.json');
  await mock.open();
...
export async function test_pullFromDevice_returns8Bands(assert) {
  ...
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  const defined = result.filters.filter(f => f !== undefined);
  assert.equal(defined.length, 8, 'SchemeNo11 CS43131 should return 8 bands');
}
...
export async function test_pullFromDevice_gainsAreNumbers(assert) {
  // SchemeNo11 declares [-12, +6] but captured values may exceed +6 if the user
  // had set a higher gain before capture. Just verify gains are finite numbers.
  const mock = await loadCapture('../captures/walkplay_schemeno11_cs43131_hifi_audio_dsp.json');
  await mock.open();
  const details = makeDeviceDetails(mock);
  const result = await walkplayUsbHID.pullFromDevice(details, 101);
  result.filters.filter(f => f !== undefined).forEach((f, i) => {
    assert.ok(typeof f.gain === 'number' && isFinite(f.gain),
      `filter[${i}].gain ${f.gain} should be a finite number`);
  });
}
